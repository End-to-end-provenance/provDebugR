...
get.expected.forward <- function()
{
	...
	# combine into list
	expected <- list(d, a, vector.1, e, f, vector.2, dev.2, 
					 vector.3, vector.4, b, g)
	names(expected) <- c("d", "a", "vector.1", "e", "f", "vector.2", "dev.2",
						 "vector.3", "vector.4", "b", "g")
...
{
	# all
	c2 <- utils::capture.output(c1 <- debug.lineage(all = TRUE, forward = TRUE))
	...
	c4 <- utils::capture.output(
		c3 <- debug.lineage("output", "a", "invalid", all = TRUE, forward = TRUE))
	
	c3$a <- c3$a[ , -3]   # omit code columns from testing
	c3$d <- c3$d[ , -3]
	c3$e <- c3$e[ , -3]
	c3$f <- c3$f[ , -3]
	c3$`dev.2` <- c3$`dev.2`[ , -3]
	c3$`vector.1` <- c3$`vector.1`[ , -3]
	c3$`vector.2` <- c3$`vector.2`[ , -3]
	c3$`vector.3` <- c3$`vector.3`[ , -3]
	c3$`vector.4` <- c3$`vector.4`[ , -3]
	
	expect_equivalent(c3, e.forward)
	expect_true(nchar(paste(c4, collapse='\n')) > 0)
	
	# start line query (valid)
	c5 <- debug.lineage(start.line = 16, all = TRUE, forward = TRUE)[[1]]
	c5 <- c5[ , -3]   # omit code column from testing
	
	e5 <- e.forward$`vector.4`
	
	expect_equivalent(c5, e5)
	
	# start line query (invalid)
	c7 <- utils::capture.output(
		c6 <- debug.lineage(start.line = 100, all = TRUE, forward = TRUE))
	...
	expect_warning(
		c9 <- utils::capture.output(
			c8 <- debug.lineage(start.line = c(1:2), all = TRUE, forward = TRUE)))
	...
	c11 <- utils::capture.output(
		c10 <- debug.lineage(script.num = 1, all = TRUE, forward = TRUE))
	
	c10$a <- c10$a[ , -3]   # omit code columns from testing
	c10$d <- c10$d[ , -3]
	c10$e <- c10$e[ , -3]
	c10$f <- c10$f[ , -3]
	c10$`dev.2` <- c10$`dev.2`[ , -3]
	c10$`vector.1` <- c10$`vector.1`[ , -3]
	c10$`vector.2` <- c10$`vector.2`[ , -3]
	c10$`vector.3` <- c10$`vector.3`[ , -3]
	c10$`vector.4` <- c10$`vector.4`[ , -3]
	...
	c13 <- utils::capture.output(
		c12 <- debug.lineage(script.num = 100, all = TRUE, forward = TRUE))
	...
	expect_warning(
		c15 <- utils::capture.output(
			c14 <- debug.lineage(script.num = c(1:2), all = TRUE, forward = TRUE)))
